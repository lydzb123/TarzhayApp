...
const ProductInfo = ({ productData }) => {
  ...
  const { price_reg, price_discount, sale_end, total_questions,
    five_star_count, four_star_count, three_star_count, two_star_count, one_star_count,
    main_text, sub_text, link_text
  } = productData;



  // Math for discounts
  const discountAmt = parseFloat(price_reg - price_discount).toFixed(2);
  const discountPercent = Math.round(discountAmt / price_reg * 100);

  //Math for star ratings
  const stars = [five_star_count, four_star_count, three_star_count, two_star_count, one_star_count];
  const totalRatings = five_star_count + four_star_count + three_star_count + two_star_count + one_star_count;
  const avgRating = parseFloat((five_star_count * 5 + four_star_count * 4 + three_star_count * 3 + two_star_count * 2 + one_star_count) / totalRatings).toFixed(1);
  ...
  return (
    <ProductInfoContainer>
      <Container>
        <h1>{totalRatings}{discountAmt}</h1>
        <SaleText>
          <Price
            sale={sale_end}>{price_discount ? `$${price_discount}` : `$${price_reg}`}
          </Price>
      ...
      <Promo>
        ...
        <a href="#" className="learn-more-link">{link_text}</a>
      </Promo>
